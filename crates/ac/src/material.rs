...
fn normal_map(m: &kn5::Material) -> Option<&str> {
    m.texture("txNormal").filter(|_| m.property("nmObjectSpace").unwrap_or(0.0) == 0.0)
}
...
impl Shading {
    fn of(m: &kn5::Material) -> Self {
        ...
        Self { specular: p("ksSpecular") / p("ksDiffuse").max(MIN_DIFFUSE), exponent: p("ksSpecularEXP"), fresnel: p("fresnelC"), max_level: p("fresnelMaxLevel") }
    }
    ...
    fn surface(&self, [highlight, gloss, mirror]: [f32; 3]) -> Surface {
        ...
        let f0_reflection = if self.max_level > 0.0 { self.fresnel.max(REFLECTIVE_F0) * mirror } else { 0.0 };
        ...
        Surface { roughness, reflectance: (f0 / 0.16).sqrt(), reflection }
    }
}
...
fn bake_surface(maps: &Image, shading: Shading) -> Image {
    ...
    let pixels = maps
        ...
        .flat_map(|p| {
            ...
            [byte(s.reflection), byte(s.roughness), 0, byte(s.reflectance / max_reflectance())]
        })
        .collect();
    Image { width: maps.width, height: maps.height, pixels }
}
...
impl Job {
    fn run(self, data: &[u8]) -> Result<Vec<u8>, String> {
        match self {
            ...
            Self::Surface(shading) => Ok(texture::encode(bake_surface(&texture::decode(data)?, shading))),
        }
    }
    ...
    fn key(self) -> JobKey {
        match self {
            ...
            Self::Surface(s) => (3, [s.specular, s.exponent, s.fresnel, s.max_level].map(f32::to_bits)),
        }
    }
}
...
fn texture_uses(m: &kn5::Material) -> Vec<(&str, Job)> {
    let mut uses: Vec<(&str, Job)> = m.texture("txDiffuse").map(|t| (t, Job::Prepare(diffuse_mips(m)))).into_iter().collect();
    uses.extend(normal_map(m).map(|t| (t, Job::Prepare(Mips::Complete))));
    uses.extend(m.texture("txMaps").map(|t| (t, Job::Surface(Shading::of(m)))));
    if is_multilayer(m) {
        let detail = std::iter::once("txMask").chain(DETAIL_LAYERS.iter().map(|(s, _)| *s));
        uses.extend(detail.filter_map(|s| m.texture(s)).map(|t| (t, Job::Prepare(Mips::Complete))));
    }
    uses
}
...
impl<'a> Materials<'a> {
    ...
    pub fn get(&mut self, visual: &mut VisualBuilder, cache: &mut TextureCache, index: usize) -> u32 {
        let Some(m) = self.kn5.materials.get(index) else {
            return *self.fallback.get_or_insert_with(|| visual.add_material(Material::default()));
        };
        if let Some(i) = self.added[index] {
            return i;
        }
        let mut texture = |name: Option<&str>, job: Job| {
            let key = self.textures.get(&(name?.to_ascii_lowercase(), job.key()))?;
        ...
        let surface = match surface_texture {
            Some(_) => Surface { roughness: 1.0, reflectance: max_reflectance(), reflection: 1.0 },
            None => shading.surface([1.0; 3]),
        };
        let detail = is_multilayer(m).then(|| detail(m, &mut |s, mips| texture(m.texture(s), Job::Prepare(mips)))).flatten();
...
mod tests {
    ...
    fn shaded(properties: &[(&str, f32)]) -> kn5::Material {
        kn5::Material { properties: properties.iter().map(|&(n, v)| (n.into(), v)).collect(), ..Default::default() }
    }
    ...
    fn textures_follow_the_material() {
        ...
        assert_eq!(texture_uses(&m), [("leaf.dds", Job::Prepare(Mips::AlphaTest(0.5)))]);
        ...
        assert_eq!(texture_uses(&glass), [("glass.dds", Job::Prepare(Mips::Source))]);

        let samplers = ["txDiffuse", "txNormal", "txMaps", "txDetail"].iter().map(|s| (s.to_string(), format!("{s}.dds"))).collect();
        let mut multimap = kn5::Material { shader: "ksPerPixelMultiMap".into(), samplers, ..shaded(&[("ksSpecular", 0.1)]) };
        let jobs: Vec<Job> = texture_uses(&multimap).into_iter().map(|(_, j)| j).collect();
        assert_eq!(jobs, [Job::Prepare(Mips::Complete), Job::Prepare(Mips::Complete), Job::Surface(Shading::of(&multimap))]);
    ...
    fn highlights_become_roughness_and_reflectance() {
        ...
        assert_eq!(surface(&[("ksDiffuse", 0.3), ("ksSpecularEXP", 30.0)]).reflectance, 0.0);
        ...
        let s = surface(&[("ksSpecular", 0.1), ("ksDiffuse", 0.3), ("ksSpecularEXP", 60.0)]);
        assert!((0.16 * s.reflectance.powi(2) - 0.043).abs() < 0.002, "{s:?}");
        assert_eq!(s.reflection, 0.0);
        // Strong highlights are capped.
        assert_eq!(surface(&[("ksSpecular", 1.0), ("ksDiffuse", 0.1), ("ksSpecularEXP", 5.0)]).reflectance, max_reflectance());
    }

    #[test]
    fn reflections_are_sharp_and_limited_to_the_games_level() {
        let glass = Shading::of(&shaded(&[("ksDiffuse", 0.3), ("ksSpecularEXP", 10.0), ("fresnelMaxLevel", 0.2)]));
        ...
        assert!(s.roughness > 0.6 && s.reflectance == 0.0 && s.reflection == 0.0, "{s:?}");
    }

    #[test]
    fn maps_bake_into_a_surface_texture() {
        let shading = Shading::of(&shaded(&[("ksSpecular", 0.1), ("ksDiffuse", 0.3), ("ksSpecularEXP", 60.0), ("fresnelMaxLevel", 0.5)]));
        // Texel 0: full highlight and exponent, no reflection; texel 1: no highlight, a fifth
        // of the exponent, full reflection.
        let maps = Image { width: 2, height: 1, pixels: vec![255, 255, 0, 255, 0, 51, 255, 255] };
        let out = bake_surface(&maps, shading);
        let s = shading.surface([1.0, 1.0, 0.0]);
        let byte = |v: f32| (v * 255.0).round() as u8;
        assert_eq!(out.pixels[..4], [0, byte(s.roughness), 0, byte(s.reflectance / max_reflectance())]);
