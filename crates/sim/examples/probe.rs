...
fn main() {
    ...
    for k in 0..3000 {
        ...
        if k % 500 == 0 {
            let s = &car.state;
            println!(
                "t={:.2} z={:.4} v={:.4} ext={:.4?} load={:.0?} rpm={:.0}",
                s.time,
                s.position.z,
                car.speed(),
    ...
    for k in 0..60000 {
        ...
        if t100.is_none() && v >= 100.0 {
            t100 = Some(car.state.time - 3.0);
        }
        if k % 2000 == 0 {
            let s = &car.state;
            println!(
                "t={:.1} v={:.1}km/h gear={} rpm={:.0} kappa_r={:.3} load={:.0?} pitch={:.4}",
                s.time,
                v,
                s.drivetrain.gear,
                s.drivetrain.rpm(),
                s.wheels[2].kappa,
                car.telemetry.wheels.map(|w| w.load),
                s.orientation.to_euler(glam::EulerRot::ZYX).1
            );
        }
    }
